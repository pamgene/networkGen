...
#'
#' The reference PPI network -- `string_12.5` -- is built from STRING
#' v12.5. An edge is kept if it has direct, non-transferred support from
#' at least one of the `experiments`, `database`, `textmining`, or
#' `coexpression` evidence channels. The STRING combined score is scaled
#' between 0-1, then filtered for combined score `>= 0.5`. Combined score
#' is converted to cost: `cost = 1 - (combined score/1000)`. This is the
...
#' (see [string_12.5]) to interactions involving a curated set of 502
#' kinases, 107,508 rows. Same `head`/`tail`/`cost` schema. Not the
#' default -- pass explicitly as `ppi_network` when working with kinases
#' only (e.g. faster iteration during development). Not yet regenerated
#' from the current v12.5 default network.
#'
#' @format A tibble with 107,508 rows and 3 columns (`head`, `tail`, `cost`).
#' @source STRING v12, filtered the same way as
#'   [string_12.5] (an earlier build).
